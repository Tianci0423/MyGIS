...
{
    public partial class BatchFusionDialog : Window
    {
        private readonly ObservableCollection<BatchFusionTask> _tasks = new();
        ...
        public IReadOnlyList<FusionRequest> Requests { get; private set; } = Array.Empty<FusionRequest>();
        public bool ContinueOnError { get; private set; } = true;

        public BatchFusionDialog(IReadOnlyList<RasterLayerInfo>? availableLayers = null)
        {
            InitializeComponent();
        ...
        private AddTaskResult TryAddTask(string ms, string pan, string dir, bool showMessages, out string message)
        {
            ...
            if (!FusionDialog.ValidateFusionInputs(ms, pan, showMessages))
            {
                message = "输入影像不满足融合要求。";
                return AddTaskResult.ValidationFailed;
            }
            ...
            _tasks.Add(new BatchFusionTask
            {
        ...
        {
            var selected = TaskGrid.SelectedItems.Cast<BatchFusionTask>().ToList();
            if (selected.Count == 0)
                return;
        ...
        private void OnRunClick(object sender, RoutedEventArgs e)
        {
            if (_tasks.Count == 0)
            {
                MessageBox.Show(this, "请先加入至少一个融合任务。", "提示", MessageBoxButton.OK, MessageBoxImage.Warning);
                return;
            }
            ...
            for (int i = 0; i < _tasks.Count; i++)
            {
                ...
                string output = Path.Combine(
                    outputDir,
                    $"{Path.GetFileNameWithoutExtension(task.MsPath)}_fused_{suffix}.tif");

                requests.Add(new FusionRequest(
                    FusionDialog.GetPythonExe(),
                    FusionDialog.GetScriptPath(),
                    FusionDialog.GetModelPath(),
                    task.MsPath,
                    task.PanPath,
                    output,
        ...
        private sealed class BatchFusionTask : INotifyPropertyChanged
        {
            private int _index;
