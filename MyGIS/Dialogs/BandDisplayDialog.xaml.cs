...
namespace MyGIS.Dialogs
{
    ...
    public partial class BandDisplayDialog : Window
    {
        ...
        public BandDisplayDialog(int totalBands, int[] currentBands,
            StretchType currentStretch, ColorRampType currentColorRamp, bool isMultiBand)
        {
            InitializeComponent();
        ...
        {
            if (!TryReadCurrentSettings(out var newStretch, out var newRamp, out var newBands))
                return;
            ...
            if (!StretchChanged && !ColorRampChanged && !BandsChanged)
                return;
